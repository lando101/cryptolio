...
// document.body.appendChild(container);
  ...
  //   let data = ccStreamer.addEventListener('message', function (event) {
  //     // console.log('Message from server ', event.data);
  //     // let formattedData: CryptoTimeData[];
  //     // myWorker = (event.data);
  //     // let data = (JSON.parse(event.data));
  //     // if(event.data){
  //     //   formattedData.push(event.data);
  //     // }
  ...
  ngAfterViewInit() {
    ...
    this.chart.subscribeCrosshairMove(function(param: { point: { x: number; y: number; }; time: any; seriesPrices: { get: (arg0: any) => any; }; }) {
      if (param.point === undefined || !param.time || param.point.x < 0 || param.point.x > container.clientWidth || param.point.y < 0 || param.point.y > container.clientHeight) {
        ...
        console.log(price);
        console.log("THIS IS PRICE FROM THE GRID");
